/* tslint:disable */
/* eslint-disable */
/**
 * Aspect Media Engine API
 * A FastAPI application for AI media engine functionality
 *
 * The version of the OpenAPI document: 0.0.0
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime';
import type { CoreFeatureType } from './core-feature-type';
import {
    CoreFeatureTypeFromJSON,
    CoreFeatureTypeFromJSONTyped,
    CoreFeatureTypeToJSON,
    CoreFeatureTypeToJSONTyped,
} from './core-feature-type';

/**
 * API response schema for getting a single index
 * @export
 * @interface IndexGetResponse
 */
export interface IndexGetResponse {
    /**
     * 
     * @type {string}
     * @memberof IndexGetResponse
     */
    id: string;
    /**
     * 
     * @type {string}
     * @memberof IndexGetResponse
     */
    name: string;
    /**
     * 
     * @type {string}
     * @memberof IndexGetResponse
     */
    description: string | null;
    /**
     * 
     * @type {number}
     * @memberof IndexGetResponse
     */
    numAssets: number;
    /**
     * 
     * @type {number}
     * @memberof IndexGetResponse
     */
    sizeBytes: number;
    /**
     * 
     * @type {number}
     * @memberof IndexGetResponse
     */
    totalTokensUsed: number;
    /**
     * 
     * @type {number}
     * @memberof IndexGetResponse
     */
    duration: number;
    /**
     * 
     * @type {string}
     * @memberof IndexGetResponse
     */
    userId: string | null;
    /**
     * 
     * @type {Array<CoreFeatureType>}
     * @memberof IndexGetResponse
     */
    defaultFeatures: Array<CoreFeatureType>;
    /**
     * 
     * @type {boolean}
     * @memberof IndexGetResponse
     */
    isSample: boolean;
    /**
     * 
     * @type {boolean}
     * @memberof IndexGetResponse
     */
    isSampleReady: boolean;
    /**
     * 
     * @type {Date}
     * @memberof IndexGetResponse
     */
    created: Date;
    /**
     * 
     * @type {Date}
     * @memberof IndexGetResponse
     */
    updated: Date;
}

/**
 * Check if a given object implements the IndexGetResponse interface.
 */
export function instanceOfIndexGetResponse(value: object): value is IndexGetResponse {
    if (!('id' in value) || value['id'] === undefined) return false;
    if (!('name' in value) || value['name'] === undefined) return false;
    if (!('description' in value) || value['description'] === undefined) return false;
    if (!('numAssets' in value) || value['numAssets'] === undefined) return false;
    if (!('sizeBytes' in value) || value['sizeBytes'] === undefined) return false;
    if (!('totalTokensUsed' in value) || value['totalTokensUsed'] === undefined) return false;
    if (!('duration' in value) || value['duration'] === undefined) return false;
    if (!('userId' in value) || value['userId'] === undefined) return false;
    if (!('defaultFeatures' in value) || value['defaultFeatures'] === undefined) return false;
    if (!('isSample' in value) || value['isSample'] === undefined) return false;
    if (!('isSampleReady' in value) || value['isSampleReady'] === undefined) return false;
    if (!('created' in value) || value['created'] === undefined) return false;
    if (!('updated' in value) || value['updated'] === undefined) return false;
    return true;
}

export function IndexGetResponseFromJSON(json: any): IndexGetResponse {
    return IndexGetResponseFromJSONTyped(json, false);
}

export function IndexGetResponseFromJSONTyped(json: any, ignoreDiscriminator: boolean): IndexGetResponse {
    if (json == null) {
        return json;
    }
    return {
        
        'id': json['id'],
        'name': json['name'],
        'description': json['description'],
        'numAssets': json['num_assets'],
        'sizeBytes': json['size_bytes'],
        'totalTokensUsed': json['total_tokens_used'],
        'duration': json['duration'],
        'userId': json['user_id'],
        'defaultFeatures': ((json['default_features'] as Array<any>).map(CoreFeatureTypeFromJSON)),
        'isSample': json['is_sample'],
        'isSampleReady': json['is_sample_ready'],
        'created': (new Date(json['created'])),
        'updated': (new Date(json['updated'])),
    };
}

export function IndexGetResponseToJSON(json: any): IndexGetResponse {
    return IndexGetResponseToJSONTyped(json, false);
}

export function IndexGetResponseToJSONTyped(value?: IndexGetResponse | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'id': value['id'],
        'name': value['name'],
        'description': value['description'],
        'num_assets': value['numAssets'],
        'size_bytes': value['sizeBytes'],
        'total_tokens_used': value['totalTokensUsed'],
        'duration': value['duration'],
        'user_id': value['userId'],
        'default_features': ((value['defaultFeatures'] as Array<any>).map(CoreFeatureTypeToJSON)),
        'is_sample': value['isSample'],
        'is_sample_ready': value['isSampleReady'],
        'created': ((value['created']).toISOString()),
        'updated': ((value['updated']).toISOString()),
    };
}

